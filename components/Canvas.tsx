...
/* =========================
   LAZY LOADING CONFIG
========================= */
const VIEWPORT_PADDING = 500; // Extra pixels around viewport to pre-load
const NODE_WIDTH = 400; // Approximate node width
const NODE_HEIGHT = 250; // Approximate node height
...
export default function Canvas() {
  ...
  const [expandedNode, setExpandedNode] =
    useState<AIMindmapNodeData | null>(null);
  
  const [reactFlowInstance, setReactFlowInstance] =
    useState<ReactFlowInstance | null>(null);
  
  // Track all nodes (full dataset)
  ...
  const isNodeVisible = useCallback(
    (node: Node, bounds: ReturnType<typeof getViewportBounds>) => {
      if (!bounds) return true; // Show all if bounds unavailable
  ...
  }, [allNodes, allEdges, getViewportBounds, isNodeVisible]);
  ...
  /* =========================
   SYNC STORE NODES → ALL NODES (normalized)
========================= */
useEffect(() => {
  const normalizedNodes = nodes.map((node) => ({
    ...
    data: {
      ...node.data,
      onExpand: node.data?.onExpand ?? setExpandedNode,
    },
  }));
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log(`📊 Lazy Loading Stats:
        ...
        Rendered: ${((visibleNodes.length / Math.max(allNodes.length, 1)) * 100).toFixed(1)}%
          ...
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10">
            <div
              ...
              style={{
                width: `${Math.min(
                  100,
                  (visibleNodes.length / allNodes.length) * 100
                )}%`,
              }}
            />
          </div>
